...
// Produces the numbers quoted in DECISIONS.md. Not load testing - it measures
// single-query cost, which is the thing the design claims to control.
//
// Run after `npm run seed`. Run `npm run seed:skew` first for the depth section to be
// meaningful.
...
async function explain(sql: string, params: unknown[]): Promise<Plan> {
  ...
  const { rows: textRows } = await db.query<{ 'QUERY PLAN': string }>(
  ...
  return {
    ...
    // Rows actually read by the scan nodes, not rows returned. This is the number that
    // separates the two strategies: both return 20.
...
const COLUMNS = `id, product_id, quantity, unit_price, total_price, status,
...
async function main(): Promise<void> {
  ...
  if (orderCount < DEPTH + PAGE) {
    // No explicit close: the .finally at the bottom owns the pool's lifetime.
    console.log(`> Only ${orderCount} orders. Run \`npm run seed:skew\` for a meaningful depth.\n`);
    return;
  }

  // The cursor pointing DEPTH rows in. Derived with OFFSET on purpose: this is setup,
  // not the measurement, and it is the honest way to put both strategies at the same
  // position in the same ordering.
  const anchor = await db.query<{ created_at: string; id: string }>(
  ...
  // What the index is actually worth. Dropped inside a transaction that is rolled
  // back, so a failure here cannot leave the database without it.
  ...
  const sample = await db.query<{ id: number }>(
